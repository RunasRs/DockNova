...
main() {
    ...
    # Analyse séquentielle de tous les conteneurs
    ...
    echo -e "  ${LBLUE}[+]${NC} Conteneurs actifs : ${LGREEN}$running_count${NC}"
    echo -e "  ${LBLUE}[+]${NC} Conteneurs totaux : ${LBLUE}$(($running_count + ${#stopped_containers[@]}))${NC}"
    echo
    ...
    local total_images=$($DOCKER_CMD images -q 2>/dev/null | wc -l)
    local total_volumes=$($DOCKER_CMD volume ls -q 2>/dev/null | wc -l)
    ...
    local unused_volumes=$($DOCKER_CMD volume ls -f "dangling=true" -q 2>/dev/null | wc -l)
    ...
    if [[ $running_count -gt 0 ]]; then
        ...
        local score_color=$LGREEN
        local box_color=$LGREEN
        local box_msg="SECURE ENVIRONMENT"
        ...
        if [[ $total_security_issues -gt 0 ]]; then
            ...
            [[ $containers_with_root -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_root${NC} conteneur(s) exécuté(s) en root"
            [[ $containers_privileged -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_privileged${NC} conteneur(s) en mode privilégié"
            [[ $containers_with_docker_socket -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_docker_socket${NC} conteneur(s) avec accès au socket Docker ${LRED}[CRITIQUE]${NC}"
            [[ $containers_with_host_pid -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_host_pid${NC} conteneur(s) avec namespace PID host"
            [[ $containers_with_host_network -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_host_network${NC} conteneur(s) en mode réseau host"
            [[ $containers_with_sensitive_mounts -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_sensitive_mounts${NC} conteneur(s) avec montages système sensibles"
            [[ $containers_with_sensitive_vars -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_sensitive_vars${NC} conteneur(s) avec variables sensibles exposées ${LRED}[CREDENTIALS]${NC}"
            [[ $containers_with_dangerous_caps -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_dangerous_caps${NC} conteneur(s) avec capabilities DANGEREUSES ${LRED}[CAP_SYS_ADMIN, etc.]${NC}"
            ...
            [[ $containers_with_dangerous_devices -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_dangerous_devices${NC} conteneur(s) avec devices CRITIQUES exposés ${LRED}[/dev/sda, /dev/mem]${NC}"
            [[ $containers_with_cgroup_access -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_cgroup_access${NC} conteneur(s) avec config risque cgroups ${LRED}[CONTAINER ESCAPE]${NC}"
            [[ $containers_with_cloud_creds -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_cloud_creds${NC} conteneur(s) avec credentials cloud détectés ${LRED}[AWS/GCP/Azure]${NC}"
            [[ $containers_with_seccomp_disabled -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_seccomp_disabled${NC} conteneur(s) avec Seccomp DÉSACTIVÉ ${LRED}[ALL SYSCALLS]${NC}"
            [[ $containers_with_unlimited_resources -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_unlimited_resources${NC} conteneur(s) avec ressources ILLIMITÉES ${LRED}[DoS RISK]${NC}"
            [[ $containers_with_latest_tag -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_latest_tag${NC} conteneur(s) avec tag :latest ${LRED}[NON-DETERMINISTIC]${NC}"
            [[ $containers_without_pids_limit -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_without_pids_limit${NC} conteneur(s) sans PIDs limit ${LRED}[FORK BOMB]${NC}"
            ...
            [[ $containers_with_no_logging -gt 0 ]] && echo -e "  ${LRED}[!]${NC} ${LRED}$containers_with_no_logging${NC} conteneur(s) avec logging DÉSACTIVÉ ${LRED}[NO AUDIT]${NC}"
    ...
    if [[ $dangling_images -gt 0 ]] || [[ $unused_volumes -gt 0 ]] || [[ $stopped_count -gt 0 ]]; then
    ...
    # Mode CI: exit code basé sur le score
    if [[ "$CI_MODE" == "true" ]]; then
        if [[ $security_score -lt $FAIL_THRESHOLD ]]; then
            log_error "Score de sécurité ($security_score%) inférieur au seuil ($FAIL_THRESHOLD%)"
            exit 1
        else
            log_success "Score de sécurité ($security_score%) ≥ seuil ($FAIL_THRESHOLD%)"
            exit 0
        fi
    fi
